      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-7 sm:py-9 space-y-11 sm:space-y-16" dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        <section className="text-start">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-5">
            ...
            <div className="inline-flex w-full lg:w-auto rounded-2xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-1.5"><button type="button" onClick={() => setActiveWorkflow('library')} className={`flex-1 lg:flex-none rounded-xl px-4 py-3 text-xs font-black transition-colors cursor-pointer ${activeWorkflow === 'library' ? 'bg-[#0F3D5E] text-white shadow-sm' : 'text-gray-500 dark:text-gray-300 hover:text-[#0F3D5E]'}`}>{isRtl ? 'مسیر معماران و مهندسان' : 'Designer path'}</button><button type="button" onClick={() => setActiveWorkflow('brand')} className={`flex-1 lg:flex-none rounded-xl px-4 py-3 text-xs font-black transition-colors cursor-pointer ${activeWorkflow === 'brand' ? 'bg-[#087F7A] text-white shadow-sm' : 'text-gray-500 dark:text-gray-300 hover:text-[#087F7A]'}`}>{isRtl ? 'مسیر تولیدکنندگان' : 'Manufacturer path'}</button></div>
          </div>
          <div className="mt-8 relative"><div className="hidden md:block absolute top-8 right-[8%] left-[8%] h-px bg-slate-200 dark:bg-slate-700" />
            {activeWorkflow === 'library' ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 relative">{[['۱','جست‌وجو','محصول یا دستهٔ موردنیاز خود را پیدا کنید.'],['۲','بررسی','اطلاعات فنی و آبجکت BIM را مشاهده کنید.'],['۳','استفاده','فایل منتشرشده را دانلود و در پروژه استفاده کنید.'],['۴','ارتباط','در صورت نیاز با تولیدکننده ارتباط بگیرید.']].map(([n,t,d]) => <div key={n} className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5"><span className="flex w-9 h-9 items-center justify-center rounded-full bg-[#0F3D5E] text-white font-black text-sm">{n}</span><h3 className="mt-4 text-lg font-black text-[#2B2F33] dark:text-white">{t}</h3><p className="mt-2 text-xs leading-6 text-gray-500 dark:text-gray-400">{d}</p></div>)}</div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 relative">{[['۱','ثبت برند','برند خود را ثبت کنید.'],['۲','معرفی محصول','محصولات و کاتالوگ را معرفی کنید.'],['۳','فایل یا تولید BIM','فایل آماده بررسی می‌شود یا مسیر تولید هماهنگ می‌گردد.'],['۴','بررسی و انتشار','اطلاعات و فایل‌ها وارد مسیر بررسی می‌شوند.'],['۵','به‌روزرسانی','اطلاعات محصولات را به‌روز نگه دارید.']].map(([n,t,d]) => <div key={n} className="relative bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5"><span className="flex w-9 h-9 items-center justify-center rounded-full bg-[#0FB9B1] text-[#0B1220] font-black text-sm">{n}</span><h3 className="mt-4 text-lg font-black text-[#2B2F33] dark:text-white">{t}</h3><p className="mt-2 text-xs leading-6 text-gray-500 dark:text-gray-400">{d}</p></div>)}</div>}
          </div>
